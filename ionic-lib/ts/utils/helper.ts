...
export function getIssuerPublicKey(keyId: string, ddo: DidDocument): Buffer {
  let relevantKeySection = ddo.publicKey.find(section => section.id === keyId)
  if (!relevantKeySection) {
      relevantKeySection = ddo.publicKey.find(section => section.id === keyId.substring(keyId.indexOf('#')))
  }
  if (!relevantKeySection) {
